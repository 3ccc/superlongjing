...
window.tianshan = (function () {
    ...
    function readConfig() {
        console.log('in readconfig--------------------->')
        var data = _utils.getCookie(_space + _cookie.config) || {};
        return {content: data}
    }
    ...
    function readFile(data) {
        switch (data.path) {
            case 'sys/client.json':
                return {
                    content: {
                        clientNumber: '设备号：TEST_clientNumber_001',
                        sellerId: '商户号：TEST_sellerId_001',
                        pointNo: '网点号：TEST_pointNo_001'
                    }
                };
                break;
            default:
    ...
    function _alert(fnName,data,outData) {
        var msg= '执行 '+fnName+(data ? (',入参：'+JSON.stringify(data)):''); // +(outData ?(',出参：'+getReturn(outData)):'');
    ...
    function executeFn(opts) {
        ...
        switch (opts.fn) {
            ...
            case 'writeConfig':
                ...
                break;
            case 'getAppId':
                return getReturn('设备ID：TEST_APP_ID_001');
            case 'open':
                if (data.url) {
            ...
            case 'ttsSpeak':
                ...
                setTimeout(function () {
                    ljTsEventAction['TTS_SPEAK']({isEnd: true})
                }, 3000);
                break;
            case 'stopTimerTask':
                for(var i =0,len=window.ljTimer.length;i<len;i++){
                    var tItem = window.ljTimer[i];
                    if(data.taskId == tItem.id){
                        if(tItem.type == 'once'){
                            clearTimeout(tItem.timerId)
                        }else{
                            clearInterval(tItem.timerId)
                        }
                    }
                }
                break;
            case 'createTimerTask':
                // _alert('创建定时器' + JSON.stringify(data));
                if (data.isOnce) {
                    timerId = setTimeout(function () {
                        window.ljTsEventAction['TIMER_NOTIFY'](data);
                    }, (data.time||1) * 1000);
                    window.ljTimer.push({
                        type:'once',
                        id:data.id,
                ...
                break;
            case 'getAppVersion':
                return getReturn('2.8.1.3');
            case 'changeLed':
                _alert(opts.fn,data);
                break;
            case 'getDriverVersion':
                return getReturn({version:1});
                break;
            case 'printV1':
                _alert(opts.fn,data);
                break;
            ...
            case 'takePicture':
                _alert(opts.fn,data,{
                    image:'./dd/dd/test.png'
                });
                return getReturn({
                    image:'./dd/dd/test.png'
                });
            case 'startFaceComparison':
                _alert(opts.fn,data);
                break;
            case 'stopFaceComparison':
                _alert(opts.fn);
                break;
            case 'connectPrinter':
                _alert(opts.fn,data,{
