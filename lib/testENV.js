...
window.tianshan = (function () {
    ...
    function executeFn(opts) {
        ...
        switch (opts.fn) {
            ...
            case 'changeLed':
                _alert('改变led，color:' + data.color + '，type：' + data.type);
            case 'getDriverVersion':
                return getReturn({version:1});
                break;
            case 'printV1':
                _alert('打印printV1，params:' + JSON.stringify(data.params) + '，count：' + data.count);
                break;
            case 'compareFace1V1':
                _alert('人脸识别compareFace1V1，face:' + data.face + '，idCard：' + data.idCard);
                return getReturn({
                    face:'123123',
                    code:-1
                })
